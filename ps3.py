...
def get_word_score(word, n):
    ...
    sum2 = max(1, 7*len(word) - 3*(n-len(word)))
    sum = sum1*sum2
    return sum 
...
#Expected False, but got True for word: 'honey' and hand: {'n': 1, 'h': 1, '*': 1, 'y': 1, 'd': 1, 'w': 1, 'e': 2}
    ...
    for i in word:
        if not i in hand_new or hand_new[i] <= 0:
            return False
        else:
            hand_new[i] -= 1
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        if word == "!!":            
            ...
            if is_valid_word(word, hand, word_list):
                ...
                allscore += word_score
                print(word, "earned", word_score,  "total:", allscore) 
            else:
    ...
    if word == "!!":
        ...
        print("Ran out of letters. Total score: ", allscore)
